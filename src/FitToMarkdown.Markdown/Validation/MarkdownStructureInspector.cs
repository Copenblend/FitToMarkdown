...
internal static class MarkdownStructureInspector
{
    internal static IReadOnlyList<string> Inspect(string markdown)
    {
        ...
        if (h1Count == 0)
            issues.Add("Missing H1 document title.");
        else if (h1Count > 1)
            issues.Add($"Multiple H1 headings found ({h1Count}). Expected exactly one.");
        ...
        foreach (var heading in headings)
        {
            if (heading.Level > prevLevel + 1 && prevLevel > 0)
                issues.Add($"Heading level skipped: H{prevLevel} followed by H{heading.Level}.");
        ...
        if (emptyParas > 0)
            issues.Add($"Found {emptyParas} empty paragraph(s).");
